...
describe('generate', () => {
  ...
  it('should generate deeplink without custom fields', () => {
    expect(
      ...
      `?leg-1-pickup=20+Rue+Jean+Rey%2C+75015+Paris%2C+France&leg-1-pickup-kpoi=MPH&leg-1-pickup-place_id=pickup-place_id&leg-1-pickup-time=2020-08-09T18%3A31%3A42-03%3A30&leg-1-dropoff=Mercure%2C+Paris%2C+Hotel&leg-1-dropoff-kpoi=MPH007&leg-1-dropoff-place_id=dropoff-place_id&leg-1-m-pickup-test=pickup+test&leg-1-m-pickup-second-test=pickup+second+test&leg-1-m-dropoff-test=dropoff+test&leg-1-m-dropoff-second-test=dropoff+second+test&leg-1-m-test=test&leg-1-m-second-test=second+test&email=email%40of.user&luggage=2&passengers=3&first-name=first+name&last-name=last+name&phone-number=%2B441234567890&traveller-locale=en-GB&meta.first=first+meta&meta.second=second-meta`
    )
  })
})
