import { Http, HttpResponse, DefaultRequestOptions, RequestOptions, Query } from './types'
...
export async function request<T>(url: string, options: RequestInit): Promise<HttpResponse<T>> {
  try {
    ...
    const code =
      message === 'Failed to fetch' || message.indexOf('offline') !== -1
        ? errorCodes.ERR_OFFLINE
...
export class HttpService implements Http {
  ...
  public post<T>(url: string, body: object): Promise<HttpResponse<T>> {
    return this.request<T>(url, { method: 'POST', ...toJsonBody(body) })
  }

  public put<T>(url: string, body: object): Promise<HttpResponse<T>> {
    return this.request<T>(url, { method: 'PUT', ...toJsonBody(body) })
  }
